...
GUARD_BOT_TOKEN  = "YOUR_BOT_TOKEN_HERE"   # same token as office_relay.py BOT_TOKEN
GUARD_WEBAPP_URL = "YOUR_VERCEL_URL_HERE"   # full HTTPS URL to verify.html on Vercel
...
def _send_webapp_button(user_id: int) -> int:
    """Sends the 'Verify My Device' WebApp button. Returns message_id."""
...
def _edit_remove_button(user_id: int, msg_id: int, new_text: str):
    """Replaces the verify button message with a plain result text."""
    if not msg_id:
        return
