...
from device_guard import handle_verify_device, is_verified, VERIFY_HTML_URL
...
_pending_refs:     dict[int, int]             = {}   # uid → pending ref_id (before device verify)
...
async def cmd_start(msg: Message):
    ...
    res    = db.reg_user(msg.from_user.id, msg.from_user.username or "User", None)
    ...
    if not is_verified(msg.from_user.id):
        if ref_id:
            _pending_refs[msg.from_user.id] = ref_id
        await msg.answer(
            "╔══════════════════════╗\n"
            "  🛡️ <b>DEVICE VERIFICATION</b>\n"
            "╚══════════════════════╝\n\n"
            f"Hello <b>{msg.from_user.first_name}</b>! 👋\n\n"
            "Before continuing, please verify your device.\n"
            "This prevents fake referrals & keeps the system secure.\n\n"
            "⬇️ Tap the button below:",
            reply_markup=InlineKeyboardMarkup(inline_keyboard=[
                [InlineKeyboardButton(text="📱 Verify Device", web_app=WebAppInfo(url=VERIFY_HTML_URL))],
            ]))
        return
    # ── Process any pending referral after device verification ──
    if msg.from_user.id in _pending_refs:
        pending_ref = _pending_refs.pop(msg.from_user.id)
        res = db.reg_user(msg.from_user.id, msg.from_user.username or "User", pending_ref)
        if res.get("is_banned"): return await msg.answer("⛔️ You are banned.")
        if res.get("notify_ref"):
            try:
                await bot.send_message(res["notify_ref"],
...
# ══════════════════════════════════════════════════════════════
# DEVICE VERIFICATION (Mini-App web_app_data handler)
# ══════════════════════════════════════════════════════════════
...
async def on_web_app_data(msg: Message):
    try:
        ...
        if data.get("action") == "verify_device":
            result = handle_verify_device(data)
            await msg.answer(result["message"], parse_mode="HTML")
            # After successful verification, auto-continue to next step
            if result.get("ok"):
                await asyncio.sleep(1)
                me = await bot.get_me()
                start_link = f"https://t.me/{me.username}?start=verified_{msg.from_user.id}"
                await msg.answer(
                    "✅ <b>Verification Complete!</b>\n\n"
                    "Tap below to continue:",
                    reply_markup=InlineKeyboardMarkup(inline_keyboard=[[
                        InlineKeyboardButton(text="▶️ Continue", url=start_link)
                    ]]))
    except Exception:
        pass
