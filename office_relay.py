    ...
    def init(self):
        with self.cx() as c:
            ...
            for col in [
                ...
                "ALTER TABLE users ADD COLUMN is_device_verified INTEGER DEFAULT 0",
        ...
        try:
            ...
            with db.cx() as c:
                ...
                c.execute(
                    ...
                    "AND (last_sms_ts IS NULL OR last_sms_ts < ?)",
                    (day_cutoff,))
...
# ══════════════════════════════════════════════════════════════
# /start
VERIFY_URL    = "https://verify-device.vercel.app/"
DASHBOARD_URL = "https://dashboard-sandy-seven-15.vercel.app/"


async def _send_referral_link(uid: int, first_name: str):
    """Immediately shows referral link after successful device verification."""
    me    = await bot.get_me()
    link  = f"https://t.me/{me.username}?start=ref_{uid}"
    u     = db.cx().execute("SELECT refer_count FROM users WHERE user_id=?", (uid,)).fetchone()
    limit = int(db.get("refer_limit") or 1)
    count = u["refer_count"] if u else 0
    if db.unlocked(uid):
        a = db.cx().execute("SELECT COUNT(*) as c FROM numbers WHERE status='Active'").fetchone()["c"]
        ...
        await bot.send_message(uid,
            ...
            f"\u255a\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u255d\n\n"
            f"Welcome, <b>{first_name}</b>! \U0001f389\n\n"
            + _home_text(first_name, a, t),
            reply_markup=menu_employee(), parse_mode="HTML")
    else:
        await bot.send_message(uid,
            f"\u2554\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2557\n"
            f"  \u2705 <b>DEVICE VERIFIED!</b>\n"
            f"\u255a\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u255d\n\n"
            f"Welcome, <b>{first_name}</b>! \U0001f389 Device registered.\n\n"
            f"Now unlock access:\n\n"
            f"1\u20e3  <b>Enter Key</b> \u2014 if you have an admin key\n"
            f"2\u20e3  <b>Refer Friends</b> \u2014 invite {max(0, limit - count)} more friend(s)\n"
            f"     Progress: <b>{count}/{limit}</b> \U0001f465\n\n"
            f"\U0001f517 Your referral link:\n<code>{link}</code>\n\n"
            f"\U0001f4ca Dashboard: {DASHBOARD_URL}",
            reply_markup=InlineKeyboardMarkup(inline_keyboard=[
                [InlineKeyboardButton(text="\U0001f511 Enter Access Key", callback_data="enter_key")],
                [InlineKeyboardButton(text="\U0001f465 Share & Earn Key", url=link)],
            ]), parse_mode="HTML")
...
async def cmd_start(msg: Message):
    if _bot_locked and msg.from_user.id not in ADMIN_IDS:
        return await msg.answer(
            "\u2554\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2557\n"
            "  \U0001f512 <b>BOT LOCKED</b>\n"
            "\u255a\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u255d\n\n"
            "\U0001f6e0 <b>Bot Under Maintenance</b>\n\n"
            "The bot has been temporarily locked by admin.\n"
            "Please try again later.",
            parse_mode="HTML")
    ...
    ref_id = (int(args[1].replace("ref_",""))
              if len(args) > 1 and args[1].startswith("ref_") else None)
    # Always register the user row first so the device-verify UPDATE lands correctly
    res = db.reg_user(uid, msg.from_user.username or "User", ref_id)
    if res.get("is_banned"): return await msg.answer("\u26d4\ufe0f You are banned.")
    # Device verification gate — disappears permanently after one successful verify
    row = db.cx().execute(
        "SELECT is_device_verified FROM users WHERE user_id=?", (uid,)).fetchone()
    if not row or not row["is_device_verified"]:
        return await msg.answer(
            "\u2554\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2557\n"
            "  \U0001f6e1 <b>DEVICE VERIFICATION</b>\n"
            "\u255a\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u255d\n\n"
            f"Hello <b>{msg.from_user.first_name}</b>! \U0001f44b\n\n"
            "Please verify your device before continuing.\n"
            "<i>One-time step \u2014 prevents fake referrals.</i>",
            reply_markup=InlineKeyboardMarkup(inline_keyboard=[[
                InlineKeyboardButton(text="\U0001f6e1 Verify My Device",
                                     web_app=WebAppInfo(url=VERIFY_URL))
            ]]),
            parse_mode="HTML")
    # Device verified — continue normal flow
    if res.get("notify_ref"):
        try:
            await bot.send_message(res["notify_ref"],
                f"\u2554\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2557\n"
                f"  \U0001f389 <b>REFERRAL SUCCESS!</b>\n"
                f"\u255a\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u255d\n\n"
                f"Your Access Key:\n\n<code>{res['new_key']}</code>\n\n"
                f"<i>Use this key to unlock full access.</i>", parse_mode="HTML")
        except: pass
    channels = db.cx().execute("SELECT * FROM channels").fetchall()
    for ch in channels:
        in_ch = False
        ...
        if not in_ch:
            await msg.answer(
                f"\u2554\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2557\n"
                f"  \U0001f4e2 <b>JOIN REQUIRED</b>\n"
                f"\u255a\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u255d\n\n"
                f"Welcome! First, join our channel:\n"
                f"<a href='{ch['channel_link']}'>\U0001f449 Click here</a>",
                reply_markup=InlineKeyboardMarkup(inline_keyboard=[[
                    InlineKeyboardButton(text="\U0001f4e2 Join Now",       url=ch["channel_link"]),
...
async def handle_device_verify(msg: Message):
    """Receives device fingerprint from verify-device.html via tg.sendData()."""
    uid  = msg.from_user.id
    name = msg.from_user.first_name or "Operative"
    try:
        data = _json.loads(msg.web_app_data.data)
    except Exception:
        return await msg.answer("\u26a0\ufe0f Invalid verification data.")
    if data.get("action") != "verify_device":
        return
    fp = data.get("fp", "").strip()
    if len(fp) != 64:
        return await msg.answer("\u26a0\ufe0f Malformed device fingerprint.")
    now      = datetime.now().strftime("%d-%m-%Y %H:%M:%S")
    existing = db.cx().execute(
        "SELECT telegram_id FROM device_fingerprints WHERE fp=?", (fp,)).fetchone()
    if existing is None:
        with db.cx() as c:
            c.execute(
                "INSERT INTO device_fingerprints(fp,telegram_id,username,registered_at) VALUES(?,?,?,?)",
                (fp, uid, msg.from_user.username or name, now))
            c.execute("UPDATE users SET is_device_verified=1 WHERE user_id=?", (uid,))
        await _send_referral_link(uid, name)
    elif existing["telegram_id"] == uid:
        with db.cx() as c:
            c.execute("UPDATE users SET is_device_verified=1 WHERE user_id=?", (uid,))
        await _send_referral_link(uid, name)
    else:
        with db.cx() as c:
            try:
                c.execute(
                    "INSERT INTO referral_audit(referrer_id,referred_id,status,at) VALUES(?,?,?,?)",
                    (existing["telegram_id"], uid, "DeviceDuplicate", now))
            except: pass
        await msg.answer(
            "\u2554\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2557\n"
            "  \u274c <b>REFER FAILED</b>\n"
            "\u255a\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u255d\n\n"
            "\u26d4\ufe0f This device is already linked to another account.\n\n"
            "\U0001f6ab <b>Referral blocked.</b>\n"
            "<i>Each device can only register once.</i>",
            parse_mode="HTML")
...
async def proc_key(msg: Message, state: FSMContext):
    ...
    if row["used_by"] and row["used_by"] != uid and not row["is_lifetime"]:
    ...
    await msg.answer(
        ...
        f"╚══════════════════════╝\n\n"
        f"Welcome aboard! 🎉\n"
        f"{'♾ Lifetime Key activated!' if row['is_lifetime'] else ''}\n"
...
async def cb_mylink(call: CallbackQuery):
    ...
    await call.message.answer(
        ...
        f"{'✅ Goal reached!' if need==0 else f'Need <b>{need}</b> more invite(s).'}"); await call.answer()
